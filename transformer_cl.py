...
class src_2Dembeddings(nn.Module):
    def __init__(self, n_units, pretrained=True, image_size=224, network_type='mb2', channels=3, freeze_cnn=False):
        ...
        elif self.network_type == 'clip':
            # full CLIP model; we'll use encode_image(...)
            self.clip_model, _, _ = open_clip.create_model_and_transforms(
                'ViT-B-32', pretrained='openai'
            )
            self.clip_feat_dim = self.clip_model.visual.output_dim  # e.g. 768

            # If CLIP dim != transformer hidden size, add projection
            if self.clip_feat_dim != self.n_units:
                self.proj = nn.Linear(self.clip_feat_dim, self.n_units)
            else:
        ...
        # If CLIP feature dim != transformer hidden size, add a projection
        if hasattr(self, 'clip_feat_dim'):
            in_dim = self.clip_feat_dim
        else:
            # For the old CNNs we assume the last layer already matches n_units
            in_dim = self.n_units

        if in_dim != self.n_units:
            self.proj = nn.Linear(in_dim, self.n_units)
        else:
            self.proj = nn.Identity()
        ...
        # Drop the classification layer for CNN architectures (but NOT for CLIP)
        elif self.network_type != 'clip':
            modules = list(self.network.children())[:-1]
            self.network = nn.Sequential(*modules)
    ...
    def forward(self, x):
        ...
        # ---------- CLIP path ----------
        if self.network_type == 'clip':
            # CLIP expects [N, 3, H, W]; assume x is already normalized similarly
            # encode_image runs the full visual tower + pooling and returns [N, D]
            with torch.set_grad_enabled(not self.freeze_cnn):
                feature_vec = self.clip_model.encode_image(x)  # [B*T, clip_feat_dim]

            # Project to transformer hidden size if needed
            frame_embeddings = self.proj(feature_vec)  # [B*T, n_units]

            # Reshape to [batch, seq_len, n_units]
            frame_embeddings = frame_embeddings.view(batch_size, seq_len, self.n_units)

            # For compatibility, we can return feature_vec as "feature_map"
            feature_map = feature_vec

            return frame_embeddings, feature_map, self.gradients

        # ---------- non-CLIP path (your old code) ----------
        feature_map = self.network(x)

        if self.network_type == 'vit':
            # ViT already outputs [B*T, D]
        ...
        if self.network_type == 'vit':
            frame_embeddings = frame_embeddings.reshape(batch_size, seq_len, self.n_units)
        else:
            frame_embeddings = frame_embeddings.view(batch_size, seq_len, self.n_units)
...
class FullTransformer(nn.Module):
    def __init__(self, encoder, src_emb, output, position, num_classes, classifier_hidden_dim, hidden_size,dropout, proj_dim=128):
        ...
        # IMPROVED: Better projection head for contrastive learning
        # Use 3 layers with gradual dimension reduction
        # intermediate_dim = (hidden_size + proj_dim) // 2

        # self.projection_head = nn.Sequential(
        #     nn.Linear(hidden_size, intermediate_dim),
        #     nn.BatchNorm1d(intermediate_dim),  # Add BN for stability
        #     nn.ReLU(),
        #     nn.Dropout(self.dropout),
        #     nn.Linear(intermediate_dim, proj_dim),
        #     # No activation here - normalize after
        # )

        self.projection_head = nn.Linear(hidden_size, proj_dim)
    ...
    def forward(self, src, src_mask, rel_mask=None, hand_seqs=None):
        ...
        # NEW: projection for contrastive learning
        proj = self.projection_head(pooled_features)      # [batch_size, proj_dim]
        # proj = F.normalize(proj, dim=1)                   # L2-normalize
        proj = F.normalize(proj, p=2, dim=1, eps=1e-12)

        # Select the best sign (most likely class) for each sample in the batch
        #class_logits = torch.argmax(class_logits, dim=1)  # Shape: [batch_size]
        ...
        return comb_out, class_logits, hand_out, proj


#Create the full model
def make_model(num_classes, n_stacks=2, n_units=512, n_heads=8, d_ff=2048, dropout=0.3, image_size=224, pretrained=True,
                emb_type='2d', emb_network='mb2', full_pretrained=None, hand_pretrained=None, freeze_cnn=False, channels=3,
                classifier_hidden_dim=256, proj_dim=128):
    ...
    model = FullTransformer(
        ...
        src_emb=src_2Dembeddings(n_units, pretrained, image_size, network_type=emb_network, channels=channels, freeze_cnn=freeze_cnn),
        output= None,
        position= c(position),
